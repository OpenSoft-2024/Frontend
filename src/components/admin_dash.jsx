...
function Banners() {
  const [offers, setOffers] = useState([{ image: "", product_id: "", id: "" }]);
    ...
    if (id >= 0) {
      ...
      for (const key in offer_deals_data) {
        if (offer_deals_update_data[key] != offer_deals_data[key])
          offer_deals_update = {
            ...offer_deals_update,
            [key]: offer_deals_data[key],
          };
      }
      console.log(offer_deals_update, id);
      if (Object.keys(offer_deals_update).length) {
        axios
          .patch(`${config.BASE_URL}/offers/updateOffer_deals/${id}`, {
            offer_deals_update,
            token,
          })
          .then((res) => {
      ...
      axios
        .post(`${config.BASE_URL}/offers/createOffer_deals`, {
          offer_deals_data,
          token,
        })
        .then((res) => {
  ...
  useEffect(() => {
    if (id >= 0) {
      const offers_deals_id_data = offers?.filter((item) => {
        return item.id == id;
      });
      ...
      for (const key in offers_deals_id_data[0]) {
        if (offer[key] != undefined) offer[key] = offers_deals_id_data[0][key];
      }
      // console.log(offers);
  ...
  const handleDelete = (orderid) => {
    if (orderid >= 0) {
      ...
      axios
        .delete(
          `${config.BASE_URL}/offers/deleteBanner/${orderid}/`,
          requestOptions
        )
        .then((res) => {
          console.log(res);
          toast.success(
            "Offer Deleted! It may take few minutes to reflect the changes"
          );
          // setLoading(false)
          handleClose1();
        })
          ...
          <Box sx={{ display: "flex" }}>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              ...
              <Select
                ...
                value={offer_deals_data.product_id}
              ...
              {offer_deals_data?.image != "" ? (
                <img
                  src={offer_deals_data?.image}
                  style={{
                    width: "3rem",
                    height: "3rem",
                    marginBottom: "10px",
                  }}
                  className="offer_deals_img"
                />
              ) : (
                <></>
              )}
